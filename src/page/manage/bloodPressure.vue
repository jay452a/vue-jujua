    ...
    <div class="s-header s-action-bar">
      <div class="top">
        <span class="circle">
          <span class="inner">
            <p>{{pressure}}</p>
            <p style="font-size: 12px;margin: 5px 0">{{log}}</p>
            <p>{{status}}</p>
          </span>
        </span>
...
export default {
  data() {
    return {
      ...
      testTime: '',
      listData: [
        /*{ param: '133/73', testTime: '2018-3-12: 19:00:00', status: '正常' },
        { param: '133/73', testTime: '2018-3-12: 19:00:00', status: '正常' },
        { param: '133/73', testTime: '2018-3-12: 19:00:00', status: '正常' },
        { param: '133/73', testTime: '2018-3-12: 19:00:00', status: '正常' },
        { param: '133/73', testTime: '2018-3-12: 19:00:00', status: '正常' },
  ...
  methods: {
    loadData() {
      ...
      getHealthData(params).then(res => {
        ...
        if(this.pageNo === 1) {
          this.pressure = `${res[0].RightSys > 0?res[0].RightSys:res[0].LeftSys}/${res[0].RightDia>0?res[0].RightDia:res[0].LeftDia}`
          this.status = res[0].JudgeResult
          this.testTime = res[0].CreateDate
        }
        const data = []
        res.map(res => {
          data.push({ param: {pressure:`${res.RightSys > 0?res.RightSys:res.LeftSys}/${res.RightDia>0?res.RightDia:res.LeftDia}`}, testTime: res.CreateDate, status: res.JudgeResult})
        })
